...
static char version[] __initdata = KERN_INFO "" PCIE_DMA_MODULE_NAME " :" PCIE_DMA_VERSION "\n";
...
void get_trans_list_info(void)
{
	...
	ptmp = s_pcit_dma.write.msg.busy_list_head.next;
	while (ptmp != &s_pcit_dma.write.msg.busy_list_head) {
		msg_write_count++;
		ptmp = ptmp->next;
	}

	ptmp = s_pcit_dma.write.data.busy_list_head.next;
	while (ptmp != &s_pcit_dma.write.data.busy_list_head) {
		data_write_count++;
		ptmp = ptmp->next;
	}

	ptmp = s_pcit_dma.read.msg.busy_list_head.next;
	while (ptmp != &s_pcit_dma.read.msg.busy_list_head) {
		msg_read_count++;
		ptmp = ptmp->next;
	}

	ptmp = s_pcit_dma.read.data.busy_list_head.next;
	while (ptmp != &s_pcit_dma.read.data.busy_list_head) {
		data_read_count++;
		ptmp = ptmp->next;
	}
		...
		unsigned long arg)
{
	...
	switch (cmd) {
		case HI_IOC_PCIT_INQUIRE:

		case HI_IOC_PCIT_DMARD:
			if (PCI_DMA_READ == task.dir) {
		...
		case HI_IOC_PCIT_BINDDEV:

		case HI_IOC_PCIT_DOORBELL:

		case HI_IOC_PCIT_SUBSCRIBE:

		case HI_IOC_PCIT_UNSUBSCRIBE:

		case HI_IOC_PCIT_LISTEN:
