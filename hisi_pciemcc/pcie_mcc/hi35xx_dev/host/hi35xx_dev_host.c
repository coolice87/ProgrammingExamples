...
#elif defined (HOST_ARCH_HI3536)
#include "../arch/hi3536_opt.c"
#elif defined (HOST_ARCH_HI3519)
#include "../arch/hi3519_opt.c"
#else
...
static char version[] __initdata = KERN_INFO "" HI35xx_DEV_MODULE_NAME " :" HI35xx_DEV_VERSION "\n";
...
static struct pci_device_id hi35xx_pci_tbl[] = { 
	...
	{PCI_VENDOR_HI3535, PCI_DEVICE_HI3535,
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
	{PCI_VENDOR_HI3536, PCI_DEVICE_HI3536,
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
	{PCI_VENDOR_HI3519, PCI_DEVICE_HI3519,
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
	{0,} 
...
static int __init pci_hidev_probe(struct pci_dev *pdev,
		const struct pci_device_id *pci_id)
{
	...
	if (!hi_dev->pci_bar0_virt) {
		HI_TRACE(HI35XX_ERR, "hidev[0x%x] bar0 ioremap failed!",
				g_local_handler->remote_device_number);
		ret = -ENOMEM;
	...
	if (!hi_dev->pci_bar1_virt) {
		HI_TRACE(HI35XX_ERR, "hidev[0x%x] bar1 ioremap failed!",
				g_local_handler->remote_device_number);
		ret = -ENOMEM;
	...
	return 0;
init_para_err:
#if (defined SLV_ARCH_HI3519)
	if (hi_dev->pci_bar3_virt)
		iounmap((volatile void *)hi_dev->pci_bar3_virt);
#endif
init_hidev_err:
...
static void __exit pci_hidev_remove(struct pci_dev *pdev)
{
