...
#define HI3531A_DEV_ID		0x353119e5
...
#define HI3531A_UNRESET_BIT	(1<<0)
#define HI3519_UNRESET_BIT	(1<<20)

#define MAX_BAR0_SPACE_SIZE	(0x800000)

extern void pcie_init_registers(struct regentry reg_table[],
...
#else
			printk(KERN_INFO "Starting device[0x%x][0x%x]\n",
					HI3531A_DEV_ID, hi_dev->slot_index);

			/* Set the start flag for slave hi3531a to boot from sram */
			g_local_handler->move_pf_window_in(hi_dev, SYS_CTR_REG_BASE, 0xffff, 1);
			writel(0x7964616f, (void *)(hi_dev->pci_bar1_virt + HI3531A_SYSBOOT9));
			msleep(1);

			/* clear PCIE MCC handshake flag */
			g_local_handler->move_pf_window_in(hi_dev, SYS_CTR_REG_BASE, 0xffff, 1);
			writel(0x0, (void *)(hi_dev->pci_bar1_virt + HI3531A_SYSBOOT9));
#endif
			break;
		case HI3532_DEV_ID:
			printk(KERN_INFO "Starting device[0x%x][0x%x]\n",
...
#else
			writel(0xeeeeffff, (void *)(hi_dev->pci_bar1_virt + HI3531A_SC_SYSRES));
#endif
			break;
		case HI3532_DEV_ID:
...
static int ddr_soft_training(struct hi35xx_dev *hi_dev)
{
	...
	/* map pcie window to sram in slave side */
	g_local_handler->move_pf_window_in(hi_dev, 0x0, 0xfffff, 0);

	/* copy pc jump code to slave sram base */
...
#elif defined SLV_ARCH_HI3531A
	slave_reg_read(hi_dev, CRG_REG_BASE + HI3531A_PERI_CRG32, &value);
	value &= ~HI3531A_UNRESET_BIT;
	slave_reg_write(hi_dev, CRG_REG_BASE + HI3531A_PERI_CRG32, value);
...
#elif SLV_ARCH_HI3531A
		slave_reg_read(hi_dev, SYS_CTR_REG_BASE + HI3531A_SYSBOOT9, &value);
#elif SLV_ARCH_HI3536
...
static int init_ddr(struct hi35xx_dev *hi_dev, struct boot_attr *attr)
{
	...
	if (HI3519_DEV_ID == hi_dev->device_id 
		...
		|| HI3531A_DEV_ID == hi_dev->device_id) {
