		...
		unsigned int *ret)
{
	...
	if (s_current_reg_base != (reg->reg_addr & 0xffff0000)) {
		boot_trace(BOOT_DEBUG, "read:move pf win: currentbase 0x%x, regbase 0x%x",
...
static inline void reg_write(struct regentry *reg, struct hi35xx_dev *hi_dev)
{
	...
	if (s_current_reg_base != (reg->reg_addr & 0xffff0000)) {
		boot_trace(BOOT_DEBUG, "write:move pf win: currentbase 0x%x, regbase 0x%x",
				s_current_reg_base, reg->reg_addr);
	...
	boot_trace(BOOT_DEBUG, "--------- write value:%x[%x]",
			reg_val_w,
			reg->reg_addr - s_current_reg_base);

	writel(reg_val_w, (void *)(reg->reg_addr - s_current_reg_base + hi_dev->pci_bar1_virt));
		...
		struct hi35xx_dev *hi_dev, unsigned int pm)
{
	...
	s_current_reg_base = 0;

	for (i=0; ; i++) {
		boot_trace(BOOT_DEBUG, "indx %d ---------- addr:0x%x, value:0x%x",
