...
#elif defined CONFIG_ARCH_HI3536
	#include "arch/hi3536/hi3536_boot_opt.c"
#elif defined CONFIG_ARCH_HI3519
	#include "arch/hi3519/hi3519_boot_opt.c"
#else
...
static char version[] __initdata = KERN_INFO "" PCIE_BOOT_MODULE_NAME " :" PCIE_BOOT_VERSION "\n";
	...
	for (i = 0; i < g_local_handler->remote_device_number; i++) {
		...
		if (attr->id == hi_dev->slot_index) {
			if (NEED_DDR_INIT & attr->type) {
				...
				if (hi_dev->vdd_checked_success	== DEVICE_RESTART_FLAG)	{
