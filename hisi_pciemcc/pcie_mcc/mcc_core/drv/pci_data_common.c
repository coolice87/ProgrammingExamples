...
int hisi_is_irq_buffer_empty(void *dev)
{
	...
	rp = readl((void *)p->irqmem.rp_addr);
	wp = readl((void *)p->irqmem.wp_addr);
...
int hisi_is_thread_buffer_empty(void *dev)
{
	...
	rp = readl((void *)p->threadmem.rp_addr);
	wp = readl((void *)p->threadmem.wp_addr);
...
static void * _wait_data_ready(struct hisi_memory_info *p)
{
	...
	rp = readl((void *)p->rp_addr);
	wp = readl((void *)p->wp_addr);
		...
		struct hisi_pci_transfer_head *head)
{
	...
	do {
		...
		rp = readl((void *)exchg_write_buf->rp_addr_virt);
		wp = readl((void *)exchg_write_buf->wp_addr_virt);
	...
	wp = readl((void *)exchg_write_buf->wp_addr_virt);
	writel((wp + dma_size), (void *)exchg_write_buf->wp_addr_virt);
	//barrier();
		...
		struct hisi_pci_transfer_head *head)
{
	...
	if (!g_local_handler->is_host()) {
		...
		pinfo = &hisi_host_info_map[dev->slot_index];
	}
		...
		struct hisi_shared_memory_info *p)
{
	...
	if (!g_local_handler->is_host()) {
#endif
		writel(0, (void *)(p->irqmem.rp_addr));
		writel(0, (void *)(p->irqmem.wp_addr));
		writel(0, (void *)(p->threadmem.rp_addr));
		writel(0, (void *)(p->threadmem.wp_addr));
